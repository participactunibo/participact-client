...
//       return getRestTemplate().getForObject(Configuration.REJECT_TASK_URL, ResponseMessage.class, user, password, taskId);
        ResponseEntity<ResponseMessage> response = getRestTemplate().exchange(Configuration.REJECT_TASK_URL, HttpMethod.GET, BasicAuthenticationUtility.getHttpEntityForAuthentication(context), ResponseMessage.class, taskId);
