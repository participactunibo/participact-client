...
public class TaskHiddenAvailableCard extends Card implements OnClickListener {

    private final static Logger logger = LoggerFactory.getLogger(TaskHiddenAvailableCard.class);
    ...
    public TaskHiddenAvailableCard(TaskFlat task) {
    ...
    public View getCardContent(Context context) {
        ...
        if (TaskService.isTaskCompatibleWithThisAppVersion(task)) {
            ...
            TextView deadlineTextView = (TextView) view.findViewById(R.id.deadline);
            DateTimeFormatter formatter = DateTimeFormat.forPattern("dd/MM/YYYY HH:mm");
            deadlineTextView.setText(StringUtils.formatForTextView(context.getString(R.string.card_label_deadline_date), formatter.print(task.getDeadline())));
            deadlineTextView.setTextColor(context.getResources().getColor(R.color.secondary_text));
    ...
    private class AcceptTaskRequestListener implements RequestListener<ResponseMessage> {
        ...
        public void onRequestSuccess(ResponseMessage result) {
            if (result != null && result.getResultCode() == ResponseMessage.RESULT_OK) {
                ...
                //in hidden case task was just in db -> changeTaskState not add
                ...
                if (!GeolocalizationTaskUtils.isActivatedByArea(task) || (last != null && GeolocalizationTaskUtils.isActivatedByArea(task) && GeolocalizationTaskUtils.isInside(context, last.getLongitude(), last.getLatitude(), task.getActivationArea()))) {
                    //tasks activated by area are inserted also in running state because we'll check the area in task active card
